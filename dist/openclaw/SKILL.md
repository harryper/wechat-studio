...
提示词采用“简洁单幅科普概念插画”，不向模型发送完整文章标题。生成或重生图片时必须从当前 `article.md` 提取摘要和四个章节作为五张图各自的内容依据，不得只依赖主题库中可能为空的 `key_points`；每张图只使用对应正文中明确出现的实体、动作和环境，并把实际提示词保存为 `image-prompts.json`。五张图复用统一色板、轻俯视视角、轮廓粗细和留白比例。每张图最多两个人、四个主要物体和一条纯色关系路径，至少三分之一为干净背景；不用杂志页面、信息图版面、卡片、文本框、纸张、书页、表格、坐标轴、仪表盘或屏幕，所有表面保持纯净空白。器材必须与主题领域直接相关；心理、认知和行为主题禁止使用烧瓶、试管、分子结构、化学公式、显微镜或装饰性柱状图。
