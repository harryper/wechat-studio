...
```
读取: {baseDir}/clients/{client}/style.yaml
```

如果客户目录不存在，告诉用户：
- 参考 `{baseDir}/references/style-template.md` 创建配置
...
保存到 `{baseDir}/output/{client}/{date}-{slug}.md`
...
读取: {baseDir}/references/writing-guide.md（去AI痕迹部分）
```
...
读取: {baseDir}/clients/{client}/gpt-image-2-prompts.md（如果存在，优先作为该客户的视觉覆盖层）
```
...
python3 {baseDir}/toolkit/fix_image_paths.py \
...
```bash
python3 {baseDir}/toolkit/cli.py gallery {markdown_path} --no-open -o {output_dir}/theme-gallery.html
```
告知用户画廊文件路径，等用户选主题。
...
发布成功后，向 `{baseDir}/clients/{client}/history.yaml` 追加一条记录：
...
```bash
python3 {baseDir}/scripts/fetch_stats.py --client {client} --days 7
```
...
```bash
python3 {baseDir}/scripts/build_playbook.py --client {client}
```
...
```bash
python3 {baseDir}/scripts/learn_edits.py --client {client} --draft {draft_path} --final {final_path}
```
...
```bash
python3 {baseDir}/scripts/learn_edits.py --client {client} --summarize
```
