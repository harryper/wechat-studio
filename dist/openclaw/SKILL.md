...
提示词采用“知识型编辑插画 / 现代科普杂志视觉”，不要用书名号引用标题，主题词只作为语义背景。科普信息通过人物、物体、空间关系和颜色编码表达；允许使用无文字箭头、分层、路径、对照面板和时间演变结构，但所有图形元素不加文字标签。五张图分别承担结构化概念封面、历史起源、机制图解、证据实验、应用边界五种角色，并复用统一色板与画面风格。每条提示词必须禁止标题、段落、字母、汉字、数字、水印、logo、书页、文件、标牌、屏幕界面、带读数仪表和海报排版。
