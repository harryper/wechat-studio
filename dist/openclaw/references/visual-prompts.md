...
> Web 工作台运行时优先采用“知识型编辑插画 / 现代科普杂志视觉”：允许无文字箭头、分层、路径、对照面板和时间演变结构来解释知识，但所有图形元素不加文字标签。默认五图角色依次为结构化概念封面、历史起源、机制图解、证据实验、应用边界；不得把整组配图降级成单焦点电影剧照。下文通用模板若与这条运行时原则冲突，以本原则为准。
