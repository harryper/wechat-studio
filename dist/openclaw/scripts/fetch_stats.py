#!/usr/bin/env python3
"""
Fetch WeChat article statistics and update client history.yaml.
...
Usage:
    ...
    python3 fetch_stats.py --client zhulv --days 7
...
def update_history(stats_list: list[dict], client: str):
    """Match stats to client history.yaml entries and update."""
    ...
    if isinstance(history, dict):
        ...
        print(f"Unsupported history format in {history_path}")
        return
    ...
    if updated > 0:
        with open(history_path, "w", encoding="utf-8") as f:
            yaml.dump(
                {"articles": articles} if wrapped else articles,
                f,
                allow_unicode=True,
...
def main():
    ...
    parser.add_argument("--client", default="zhulv", help="Client name under clients/")
    ...
    if all_stats:
        update_history(all_stats, args.client)
    else:
