...
def _build_provider_from_entry(entry: dict) -> ImageProvider:
    ...
    if isinstance(api_key, str):
        ...
        api_key = None if expanded_api_key == api_key and "${" in api_key else expanded_api_key
    ...
    if entry.get("base_url"):
        kwargs["base_url"] = os.path.expandvars(entry["base_url"])
    if entry.get("secret_key"):
        kwargs["secret_key"] = os.path.expandvars(entry["secret_key"])
    if entry.get("deployment"):
        kwargs["deployment"] = os.path.expandvars(entry["deployment"])
