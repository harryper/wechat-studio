...
> Web 工作台运行时采用适配 GPT Image 2 的“当代科普编辑插画”：从当前文章摘要，以及四个实际 H2 标题和对应正文提取五份内容依据，不依赖可能为空的主题 `key_points`，也不把四张内文图固定套成起源、发展、影响或反直觉场景。每张图先明确核心判断，再从真实场景、物件/档案、图标/流程/数据图解、概念隐喻或对比构图中选择最合适的形式。人物不是必需元素，只有人物行为本身能解释论点时才使用。五张图统一低饱和海军蓝、赭石、暖象牙色、轮廓、材质和光影，但避免连续使用相同的人物朝向或桌面阅读构图。允许出现少量服务于知识解释、与当前场景及正文依据直接相关的文字，不把主题名或章节名作为装饰性标签，并禁止无关文案、乱码、Logo 或水印。器材必须与主题领域直接相关，心理/认知/行为主题不得使用化学实验器材、分子公式或装饰性图表。下文通用模板若与这条运行时原则冲突，以本原则为准。
...
- **先定表达形式，再定风格**：根据内容在真实场景、物件隐喻、图标/流程/数据图解中选择最清楚的一种；人物不是默认主体。
...
2. `Visual Form`：真实场景、物件隐喻、图标/流程/数据图解中的哪一种最合适
3. `Subject`：核心人物、物件、图标或数据关系是什么
...
- 最终用于生图的提示词，默认分成 4 段以内：
  ...
  - 镜头/光线/质感
  - 负面约束
- 如果采用人物场景，必须出现具体动作和环境锚点；如果采用图解，则必须明确图标、节点、方向和关系。
- 人物不是必需元素，不得为了表现抽象概念而默认添加人物或人物阅读文件。
- 机制、流程、比较、因果或数据关系优先考虑图标和结构图；没有原文数据时不得虚构数字。
...
- **正反面约束**：仅当画面确有阅读动作时，人物视线、载体正面和文字方向必须符合真实空间关系，看不到的一面保持无字。镜头角度自由，不得固定为肩后视角；禁止镜像文字、透视方向错误的文字和悬浮说明文字。
...
- 四张内文图使用不同的视觉形式偏好：物件/档案、图标/关系图解、真实应用场景、概念隐喻/对比构图；内容适配优先于机械套用。
